...
2. `npm run build` — готовый сайт появится в `out/`. Сборка автоматически исправляет пути сегментов Next в Windows-экспорте; `npm run check:export` проверяет их перед выкладкой.
